                ...
                <table class="table" empty-response="{{trans('variables.list_is_empty')}}">
                    ...
                    @foreach($deleted_info_item as $deleted_one_info_item)
                        <tr id="{{$deleted_one_info_item->info_item_id}}">
                            <td>
                                <span>{{!empty(IfHasName($deleted_one_info_item->info_item_id, $lang_id, 'info_item')) ? IfHasName($deleted_one_info_item->info_item_id, $lang_id, 'info_item') : trans('variables.another_name')}}</span>
                            </td>
                            <td class="medium check-restore-element">
                                <input type="checkbox" class="restore-all-elements"
                                       name="restore_elements[{{$deleted_one_info_item->info_item_id}}]"
                                       value="{{$deleted_one_info_item->info_item_id}}"
                                       url="{{urlForFunctionLanguage($lang, str_slug($deleted_one_info_item->name).'/restoreInfoItem')}}">
                            </td>
                            @if($groupSubRelations->del_from_rec == 1)
                                <td class="medium check-destroy-element">
                                    <input type="checkbox" class="remove-all-elements"
                                           name="destroy_elements[{{$deleted_one_info_item->info_item_id}}]"
                                           value="{{$deleted_one_info_item->info_item_id}}"
                                           url="{{urlForFunctionLanguage($lang, str_slug($deleted_one_info_item->name).'/destroyInfoItemFromCart')}}">
                                </td>
                            @endif
                        </tr>
                    @endforeach
