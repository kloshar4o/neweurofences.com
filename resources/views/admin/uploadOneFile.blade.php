@if(!empty($modules_name))
    <div class="label-wrap">
        <label for="img">{{myTrans('Files')}} (PDF)</label>
    </div>
    <div class="input-wrap">

        <div class='file-div' data-file-type="pdf">

            <button type="button" class='btn upload-img'>
                <span class='glyphicon glyphicon-refresh-animate'>{{trans('variables.select_file')}} (PDF)</span>
            ...
            @if(empty($element_id) && empty($element_by_lang))
                ...
                {{--create--}}
                <input type="hidden" name="file[]" class="file_pdf" data-url="{{url($lang, ['back', 'upload'])}}"
            ...
            @else
                ...
                @if(!empty($element_id) && !is_null($element_id->moduleMultipleImg) && !$element_id->moduleMultipleImg->isEmpty())
                    ...
                    @foreach($element_id->moduleMultipleImg as $one_img)
                        ...
                        @if(file_exists("upfiles/" . $modules_name->modul esId->alias . "/" . $one_img->img) && !empty($one_img->img))
