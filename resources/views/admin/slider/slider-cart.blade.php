                ...
                <table class="table" empty-response="{{trans('variables.list_is_empty')}}">
                    ...
                    @foreach($deleted_banner_list as $deleted_banner)
                        <tr id="{{$deleted_banner->banner_top_id}}">
                            <td>
                                <span>{{!empty(IfHasName($deleted_banner->banner_top_id, $lang_id, 'banner_top')) ? IfHasName($deleted_banner->banner_top_id, $lang_id, 'banner_top') : trans('variables.another_name')}}</span>
                            </td>
                            <td class="medium check-restore-element">
                                <input type="checkbox" class="restore-all-elements"
                                       name="restore_elements[{{$deleted_banner->banner_top_id}}]"
                            ...
                            @if($groupSubRelations->del_from_rec == 1)
                                <td class="medium check-destroy-element">
                                    <input type="checkbox" class="remove-all-elements"
                                           name="destroy_elements[{{$deleted_banner->banner_top_id}}]"
                                           value="{{$deleted_banner->banner_top_id}}"
