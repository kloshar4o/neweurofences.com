                ...
                <table class="table" id="tablelistsorter" empty-response="{{trans('variables.list_is_empty')}}">
                    ...
                    @foreach($banner_list as $key => $banner)
                        <tr id="{{$banner->banner_top_id}}">
                            <td>
                                <span>{{ !empty(IfHasName($banner->banner_top_id, $lang_id, 'banner_top')) ? IfHasName($banner->banner_top_id, $lang_id, 'banner_top') : trans('variables.another_name')}}</span>
                            </td>
                            <td class="edit-links">
                                @foreach($lang_list as $lang_key => $one_lang)
                                    <a href="{{urlForFunctionLanguage($lang, str_slug($banner->name).'/edititem/'.$banner->banner_top_id.'/'.$one_lang->id)}}" {{ !empty(IfHasName($banner->banner_top_id, $one_lang->id, 'banner_top')) ? 'class=active' : ''}}>{{$one_lang->lang}}</a>
                                @endforeach
                            </td>
                            @if($groupSubRelations->active == 1)
                                <td class="small active-link">
                                    <a href=""
                                       class="change-active{{$banner->bannerTopId->active == 1 ? ' active' : ''}}"
                                       data-active="{{$banner->bannerTopId->active}}"
                                       element-id="{{$banner->bannerTopId->id}}"></a>
                                </td>
                            @endif
                            ...
                            @if($groupSubRelations->del_to_rec == 1)
                                <td class="check-destroy-element">
                                    <input type="checkbox" class="remove-all-elements"
                                           name="destroy_elements[{{$banner->banner_top_id}}]"
                    ...
                    <tr>
                        <td colspan="10">
                            @include('admin.pagination', ['paginator' => $slider_list_ids])
                        </td>
                    </tr>
                    </tfoot>
                </table>
